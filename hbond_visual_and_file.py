...
def list_hb(selection,selection2=None,cutoff=3.2,angle=55,mode=1,output='hbonds'):
	"""
	USAGE
	...
	stored.hb_list = []
	for pairs in hb:
		cmd.iterate("%s and index %s" % (pairs[0][0],pairs[0][1]), 'stored.hb_list.append("%1s,%3s,%s,%-4s," % (chain,resn,resi,name)),')
		cmd.iterate("%s and index %s" % (pairs[1][0],pairs[1][1]), 'stored.hb_list.append("%1s,%3s,%s,%-4s," % (chain,resn,resi,name)),')
		stored.hb_list.append("%.2f" % cmd.distance(output,"%s and index %s" % (pairs[0][0],pairs[0][1]),"%s and index %s" % (pairs[1][0],pairs[1][1])+',') +'\n')

	outputFile = open(output+".csv", "w")
	for entry in stored.hb_list:
		outputFile.write(entry)
	outputFile.close()

	stored.outputFile.close()
